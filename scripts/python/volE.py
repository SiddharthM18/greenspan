...
def main(args):
    ...
    if 'dateRange=' in parameters[1]:
        ...
        image_file = plot_vol(ticker, volatility_elasticity(
            ticker, dates, custom_range=True))
    else:
        image_file = plot_vol(ticker, volatility_elasticity(
            ticker, int(parameters[1]), custom_range=False))
...
def plot_vol(ticker, df):
    ...
    ax1.plot(df.Date, df.Real_Vol, label=ticker +
             ' Realized Volatility', color='red')
    ax2.bar(df.Date, df.Vol_Elasticity, label=ticker +
            ' Volatilisitic Elasticity of Returns', color='blue')
...
def volatility_elasticity(ticker=str, lookback_days='dateRange', custom_range=bool):
    ...
    def import_data(ticker=str, lookback_days=int):
        ...
        def returns(df):
            ...
            df['Abs_Return'] = [price-df['Close'][n-1] if n >=
                                1 else 0 for n, price in enumerate(df.Close)]
    ...
    def add_volatility(df, period=str):
        ...
        df['Real_Vol'] = df['Log_Return'].rolling(
            window=window).std(ddof=0)*np.sqrt(252)
        ...
        df['Vol_Elasticity_Dummy'] = [
            1 if abs(vol_e) > 1 else 0 for vol_e in df.Vol_Elasticity]
